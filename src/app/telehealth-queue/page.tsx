...
export default function TelehealthQueue() {
  ...
  const {
    myBookings: originalBookings,
    isLoading,
    updateBookingStatus: globalUpdateBookingStatus,
  } = useBookings()
  ...
  const [selectedPatient, setSelectedPatient] =
    useState<BookingWithPatient | null>(null)
  ...
  const providers = [
    ...new Set(bookings.filter(b => b.provider_name).map(b => b.provider_name)),
  ]
  ...
  const getFilteredBookings = () => {
    ...
    if (filters.statuses.length > 0) {
      filtered = filtered.filter(b =>
        filters.statuses.includes(b.status as TelehealthStatus)
      )
    }
  ...
  const getGroupedBookings = () => {
    ...
    const readyForProvider = filteredBookings.filter(
  ...
  const updateBookingStatus = async (
    ...
    try {
      ...
      alert(
        `Failed to update patient status to ${newStatus}.\n\nError: ${errorMessage}\n\nPlease check the browser console for more details.`
      )
    } finally {
      setIsUpdating(null)
    }
  ...
  const renderPatientCard = (booking: BookingWithPatient) => {
    const canIntake = ['pending', 'confirmed', 'intake'].includes(
      booking.status
    )
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2">
          ...
          {canCompleteCall && (
            <Button
              ...
              variant="outline"
              onClick={() =>
                updateBookingStatus(booking.id, 'ready-for-discharge')
              }
              disabled={isUpdating === booking.id}
            >
              End Call
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {canIntake && (
                <DropdownMenuItem
                  onClick={() => updateBookingStatus(booking.id, 'intake')}
                >
                  Move to Intake
                </DropdownMenuItem>
              )}
              ...
              {booking.status === 'intake' && (
                <DropdownMenuItem
                  onClick={() =>
                    updateBookingStatus(booking.id, 'ready-for-provider')
                  }
                >
                  Ready for Provider
                </DropdownMenuItem>
              ...
              {booking.status === 'ready-for-discharge' && (
                <DropdownMenuItem
                  onClick={() => updateBookingStatus(booking.id, 'discharged')}
                >
                  Discharge Patient
                </DropdownMenuItem>
              )}
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage patient appointments and telehealth workflow
            </p>
          </div>
              ...
              <div className="flex-1 min-w-48">
                <Select
                  ...
                  onValueChange={value =>
                    setFilters(prev => ({
                      ...prev,
                      providerName: value === 'all-providers' ? null : value,
                    }))
                  }
                >
                  <SelectTrigger>
              ...
              <div className="flex-1 min-w-48">
                <Input
                  ...
                  onChange={e =>
                    setFilters(prev => ({
                      ...prev,
                      patientNameSearch: e.target.value,
                    }))
                  }
                />
              </div>
        ...
        >
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="in-office">
              ...
              {
                bookings.filter(b =>
                  ['intake', 'ready-for-provider', 'provider'].includes(
                    b.status
                  )
                ).length
              }
          ...
          <TabsContent value="in-office" className="mt-6">
            {groupedBookings && (
              <div className="space-y-6">
                ...
                {Object.values(groupedBookings).every(
                  group => group.count === 0
                ) && (
                  <Card>
                    <CardContent className="p-8 text-center">
