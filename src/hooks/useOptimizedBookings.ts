...
import type { BookingWithPatient, TelehealthStatus, QueueTab } from '@/lib/database.types'
...
export function useOptimizedBookings() {
  ...
  const stats = useMemo(() => {
    const allStatuses: TelehealthStatus[] = [
      'pending', 'confirmed', 'intake', 'ready-for-provider', 
      'provider', 'ready-for-discharge', 'discharged'
    ]
  ...
  const groupedInOfficeBookings = useMemo(() => {
    ...
    return {
      ...
      readyForProvider: inOfficeBookings.filter(b => b.status === 'ready-for-provider'),
